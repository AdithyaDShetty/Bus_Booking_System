...
AFTER INSERT ON booked FOR EACH ROW
BEGIN
  DECLARE schedule_id INT;
  DECLARE quantity INT;
  
  -- Get the schedule_id and quantity of the booking
  SELECT NEW.schedule_id, NEW.qty INTO schedule_id, quantity;
...
CREATE FUNCTION CalculateTotalPrice(scheduleId INT, quantity INT) RETURNS DECIMAL(10, 2)
BEGIN
  RETURN (SELECT price * quantity FROM schedule_list WHERE id = scheduleId);
END;
//
...
BEGIN
  INSERT INTO schedule_list (bus_id, from_location, to_location, departure_time, eta, availability, price, date_updated)
  VALUES (busId, fromLocation, toLocation, departureTime, eta, availability, price, NOW());
END;
//
...
SELECT
  ...
  b.name AS bus_name,
  l1.terminal_name AS from_terminal,
  l2.terminal_name AS to_terminal,
  sl.departure_time,
  sl.eta,
  sl.availability,
  sl.price,
...
FROM
  schedule_list AS sl
  INNER JOIN bus AS b ON sl.bus_id = b.id
  INNER JOIN location AS l1 ON sl.from_location = l1.id
  INNER JOIN location AS l2 ON sl.to_location = l2.id
  LEFT JOIN booked AS bk ON sl.id = bk.schedule_id
GROUP BY sl.id;
